...
/**
 * POST /api/conversations/:id/outputs/:outputId/reopen
 *
 * Reopens a drafted output by setting status back to PENDING
 */
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error("Error reopening output:", error);
    return NextResponse.json(
      { error: { code: "INTERNAL_ERROR", message: "Failed to reopen output" } },
      { status: 500 }
    );
  }
}
