...
export async function sendSummaryEmail(
  ...
  const emailData: SummaryEmailData = {
    ...
    meetingUrl: `${process.env.NEXT_PUBLIC_APP_URL || "https://app.inkra.io"}/meetings/${meeting.id}`,
  };
...
async function sendWithSendGrid(
  ...
  const fromEmail = process.env.SENDGRID_FROM_EMAIL || "meetings@inkra.io";
  ...
  const response = await fetch("https://api.sendgrid.com/v3/mail/send", {
    ...
    body: JSON.stringify({
      ...
      from: { email: fromEmail, name: "Inkra Meetings" },
      subject,
      content: [
...
async function sendWithSES(
  ...
  const fromEmail = process.env.AWS_SES_FROM_EMAIL || "meetings@inkra.io";
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <div style="margin-top: 32px; padding-top: 24px; border-top: 1px solid #eee; text-align: center;">
    ...
    <p style="color: #999; font-size: 12px; margin-top: 16px;">
      This summary was automatically generated by Inkra Meeting Intelligence.
    </p>
  </div>
</body>
</html>
